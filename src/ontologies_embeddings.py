...
def get_xml_data(dir):
    ...
    # Import function to process a directory with Biocreative VII files
    ...
    neg_gv, pos_gv = get_sdp_instances(dir, parser = 'spacy')
    ...
    return labels, (left_instances,
                    right_instances), classes, shared_ancestors, (
    ...
    for i, seq in enumerate(x_data):
        ...
        for d in seq:
            if d and d.startswith('CHEBI') or d.startswith('GO'):
                ...
                if d.replace(
                        '_', ':'
                ) not in id_to_index:  # 18 / 20 cases not in the ontology (probably the annotations are not updated)
                    pass
...
def get_w2v(file_name='{}/PubMed-w2v.bin'.format(data_directory)):
    ...
    word_vectors = KeyedVectors.load_word2vec_format(
        file_name, binary=True)  # C text format
...
def concatenation_ancestors(pair_type, x_subpaths_train, list_order):
    ...
    if entity_left == entity_right:
        ...
        is_a_graph, name_to_id, synonym_to_id, id_to_name, id_to_index = eval(
            dict_type_entity_ontology[entity_left])
        id_to_index_entity_left = id_to_index_entity_right = id_to_index

    else:

        is_a_graph, name_to_id, synonym_to_id, id_to_name, id_to_index_entity_left = eval(
            dict_type_entity_ontology[entity_left])
        is_a_graph, name_to_id, synonym_to_id, id_to_name, id_to_index_entity_right = eval(
            dict_type_entity_ontology[entity_right])
        id_to_index = {**id_to_index_entity_left, **id_to_index_entity_right}

    x_ids_left = preprocess_ids(x_subpaths_train[0], id_to_index_entity_left,
                                max_ancestors_length)
    x_ids_right = preprocess_ids(x_subpaths_train[1], id_to_index_entity_right,
                                 max_ancestors_length)
...
def write_plots(history, model_name, pair_type):
    ...
    plt.savefig(results_directory + pair_type + '/' +
                '{}_acc.png'.format(model_name))
        ...
        val_predict = (np.asarray(
            self.model.predict(
                [self.validation_data[i]
                 for i in range(self.n_inputs)], ))).round()
        val_targ = self.validation_data[self.n_inputs]

        self._val_f1 = f1_score(val_targ[..., 1:],
                                val_predict[..., 1:],
                                average='macro')
        self._val_recall = recall_score(val_targ[..., 1:],
                                        val_predict[..., 1:],
                                        average='macro')
        self._val_precision = precision_score(val_targ[..., 1:],
                                              val_predict[..., 1:],
                                              average='macro')
        _confusion_matrix = confusion_matrix(val_targ.argmax(axis=1),
...
def join_channels(model_name, pair_type, channels, y_train, train_labels,
                  n_classes, x_words_train, x_wordnet_train, x_subpaths_train,
                  x_ancestors_train, id_to_index):
    """
    ...
    if os.path.isfile('{}/{}.json'.format(
            models_directory + '/' + pair_type.replace('-', '_').lower(),
            model_name)):
        os.remove('{}/{}.json'.format(
            models_directory + '/' + pair_type.replace('-', '_').lower(),
            model_name))

    if os.path.isfile('{}/{}.h5'.format(
            models_directory + '/' + pair_type.replace('-', '_').lower(),
            model_name)):
        os.remove('{}/{}.h5'.format(
            models_directory + '/' + pair_type.replace('-', '_').lower(),
    ...
    if 'concatenation_ancestors' in channels:  ##### 3RD CHANNEL
        ...
        x_left, x_right, id_to_index = concatenation_ancestors(
            pair_type, x_subpaths_train, list_order)
    ...
    if 'common_ancestors' in channels:  ##### 4RT CHANNEL
        ...
        if pair_type.split('-')[0] == pair_type.split('-')[
                1]:  # only for same type entities, for instance, DRUG-DRUG
            x_common, id_to_index = common_ancestors(pair_type,
                                                     x_ancestors_train,
                                                     list_order)
            ...
            print(
                'The type of the entities participating in the pair is different, it is not possible to use the common_ancestors channel.'
            )
    ...
    with open(
            '{}/{}.json'.format(
                models_directory + '/' + pair_type.replace('-', '_').lower(),
                model_name), 'w') as json_file:
        json_file.write(model_json)

    metrics = Metrics(train_labels, x_words_train, n_inputs)

    checkpoint = ModelCheckpoint(filepath='{}/{}.h5'.format(
        models_directory + '/' + pair_type.replace('-', '_').lower(),
        model_name),
                                 verbose=1,
                                 save_best_only=True)
...
def predict(model_name, corpus_name, gold_standard, channels, test_labels,
            x_words_test, x_wn_test, x_subpaths_test, x_ancestors_test,
            id_to_index):
    """
    ...
    if 'words' in channels:  ##### 1ST CHANNEL
        ...
        x_words_test_left = preprocess_sequences([['entity'] + x[1:]
                                                  for x in x_words_test[0]],
                                                 word_vectors)
        x_words_test_right = preprocess_sequences(
            [x[:-1] + ['entity'] for x in x_words_test[1]], word_vectors)
    ...
    if 'wordnet' in channels:  ##### 2ND CHANNEL
        ...
        x_wordnet_test_left = preprocess_sequences_glove(
            x_wn_test[0], wn_index)
        x_wordnet_test_right = preprocess_sequences_glove(
            x_wn_test[1], wn_index)
    ...
    json_file = open(
        '{}/{}.json'.format(
            models_directory + '/' + corpus_name.replace('-', '_'),
            model_name), 'r')
    ...
    # Load weights
    loaded_model.load_weights('{}/{}.h5'.format(
        models_directory + '/' + corpus_name.replace('-', '_'), model_name))

    print('Loaded model {}/{} from disk.'.format(
        models_directory + '/' + corpus_name.replace('-', '_'), model_name))
    ...
    with open(
            results_directory +
            corpus_name.replace('-', '_') + '/{}_{}_results.txt'.format(
                model_name, corpus_name.replace('-', '_')), 'w') as f:
        f.write('\t'.join(['Entity_1', 'Entity_2', 'Predicted_class\n']))

        for i, pair in enumerate(test_labels):
            f.write('\t'.join(
                (pair[0], pair[1],
                 pair_type_to_label[(np.argmax(scores[i]))] + '\n')))
    '''# XML Results
...
def main():
    ...
    pair_type = sys.argv[2]  # DRUG-GENE
    ...
    if type_of_action == 'preprocess':
        ...
        train_labels, x_train, y_train, x_train_ancestors, x_train_subpaths, x_train_wordnet = get_xml_data(
            corpora_directory)

        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + '_y.npy', y_train)
        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + '_labels.npy', train_labels)
        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + '_x_words.npy', x_train)
        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + '_x_wordnet.npy', x_train_wordnet)
        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + '_x_subpaths.npy', x_train_subpaths)
        np.save(
            temporary_directory + pair_type.replace('-', '_').lower() + '/' +
            preprocess_what + "_x_ancestors.npy", x_train_ancestors)

    ##### TRAIN #####
    elif type_of_action == 'train':
        model_name = sys.argv[3]  # model_1, model_2 etc.
        channels = sys.argv[
            4:-1]  # channels to use or string with only one channel

        y_train = np.load(temporary_directory +
                          pair_type.replace('-', '_').lower() + '/' +
                          type_of_action + '_y.npy',
                          allow_pickle=True)
        train_labels = np.load(temporary_directory +
                               pair_type.replace('-', '_').lower() + '/' +
                               type_of_action + '_labels.npy',
                               allow_pickle=True)
        x_words_train = np.load(temporary_directory +
                                pair_type.replace('-', '_').lower() + '/' +
                                type_of_action + '_x_words.npy',
                                allow_pickle=True)
        x_wordnet_train = np.load(temporary_directory +
                                  pair_type.replace('-', '_').lower() + '/' +
                                  type_of_action + '_x_wordnet.npy',
                                  allow_pickle=True)
        x_subpaths_train = np.load(temporary_directory +
                                   pair_type.replace('-', '_').lower() + '/' +
                                   type_of_action + '_x_subpaths.npy',
                                   allow_pickle=True)
        x_ancestors_train = np.load(temporary_directory +
                                    pair_type.replace('-', '_').lower() + '/' +
                                    type_of_action + '_x_ancestors.npy',
                                    allow_pickle=True)

        join_channels(model_name, pair_type, channels, y_train, train_labels,
                      n_classes, x_words_train, x_wordnet_train,
    ...
    ##### TEST / PREDICT #####
        ...
        channels = sys.argv[
            5:-1]  # channels to use or string with only one channel

        id_to_index = {}
        if pair_type.split('-')[0] == 'DRUG' or pair_type.split(
                '-')[-1] == 'DRUG':
            is_a_graph_chebi, name_to_id_chebi, synonym_to_id_chebi, id_to_name_chebi, id_to_index_chebi = ontology_preprocessing.load_chebi(
                '{}/chebi.obo'.format(data_directory))
            id_to_index = {**id_to_index, **id_to_index_chebi}

        if pair_type.split('-')[0] == 'GENE' or pair_type.split(
                '-')[-1] == 'GENE':
            is_a_graph_go, name_to_id_go, synonym_to_id_go, id_to_name_go, id_to_index_go = ontology_preprocessing.load_go(
                '{}/go.obo'.format(data_directory))
            id_to_index = {**id_to_index, **id_to_index_go}

        test_labels = np.load(temporary_directory +
                              pair_type.replace('-', '_').lower() + '/' +
                              type_of_action + '_labels.npy',
                              allow_pickle=True)
        x_words_test = np.load(temporary_directory +
                               pair_type.replace('-', '_').lower() + '/' +
                               type_of_action + '_x_words.npy',
                               allow_pickle=True)
        x_wordnet_test = np.load(temporary_directory +
                                 pair_type.replace('-', '_').lower() + '/' +
                                 type_of_action + '_x_wordnet.npy',
                                 allow_pickle=True)
        x_subpaths_test = np.load(temporary_directory +
                                  pair_type.replace('-', '_').lower() + '/' +
                                  type_of_action + '_x_subpaths.npy',
                                  allow_pickle=True)
        x_ancestors_test = np.load(temporary_directory +
                                   pair_type.replace('-', '_').lower() + '/' +
                                   type_of_action + '_x_ancestors.npy',
                                   allow_pickle=True)

        predict(model_name, pair_type.lower(), gold_standard, channels,
                test_labels, x_words_test, x_wordnet_test, x_subpaths_test,
        ...
        print(
            'The type of action was not properly defined, it has to be preprocess, train or test.'
        )
